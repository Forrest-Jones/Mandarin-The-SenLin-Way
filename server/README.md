...
- Stripe: product, the three prices, the webhook and the customer portal exist and their ids live in KV (created by the deploy workflow). The stored key is a **live** restricted key (`rk_live_…`), so the Pro page charges real cards. For a rehearsal with card `4242 4242 4242 4242`, temporarily store an `sk_test_` key and rerun *Deploy API worker*; the setup repeats itself per mode.
